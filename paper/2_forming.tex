...
\begin{listing}[!htb]
\caption{Фрагмент DQL-схемы датасета Elliptic++ Transactions.}
...
метке 25.
\begin{listing}[!htb]
\caption{Запрос \ref{query1} для датасета Elliptic++ Transactions.}
\inputminted[frame=single,fontsize=\footnotesize,linenos,breaklines,xleftmargin=1.5em, breaksymbol=""]{text}
{lst/elliptic/query1}
...
легальность которых не определена --- это транзакции со значением 3 предиката \texttt{class}. % 77% от всех транзакций
\begin{listing}[!htb]
\caption{Запрос \ref{query2} для датасета Elliptic++ Transactions.}
\inputminted[frame=single,fontsize=\footnotesize,linenos,breaklines,xleftmargin=1.5em, breaksymbol=""]{text}
{lst/elliptic/query2}
...
\begin{listing}[!htb]
\caption{Запрос \ref{query3} для датасета Elliptic++ Transactions.}
\inputminted[frame=single,fontsize=\footnotesize,linenos,breaklines,xleftmargin=1.5em, breaksymbol=""]{text}
{lst/elliptic/query3}
...
\begin{listing}[!htb]
\caption{Запрос \ref{query4} для датасета Elliptic++ Transactions.}
\inputminted[frame=single,fontsize=\footnotesize,linenos,breaklines,xleftmargin=1.5em, breaksymbol=""]{text}
{lst/elliptic/query4}
...
метке.
\begin{listing}[!htb]
\caption{Запрос \ref{query5} для датасета Elliptic++ Transactions.}
\inputminted[frame=single,fontsize=\footnotesize,linenos,breaklines,xleftmargin=1.5em, breaksymbol=""]{text}
{lst/elliptic/query5}
...
\begin{listing}[!htb]
\caption{DQL-схема датасета MOOC User Actions.}
\inputminted[frame=single,fontsize=\footnotesize,linenos,breaklines,xleftmargin=1.5em, breaksymbol=""]{text}
{lst/mooc/schema}
...
\begin{listing}[!htb]
\caption{Запрос \ref{query1} для датасета MOOC User Actions.}
\inputminted[frame=single,fontsize=\footnotesize,linenos,breaklines,xleftmargin=1.5em, breaksymbol=""]{text}
{lst/mooc/query1}
...
% 95% всех пользовательских действий
\begin{listing}[!htb]
\caption{Запрос \ref{query2} для датасета MOOC User Actions.}
\inputminted[frame=single,fontsize=\footnotesize,linenos,breaklines,xleftmargin=1.5em, breaksymbol=""]{text}
{lst/mooc/query2}
...
\begin{listing}[!htb]
\caption{Запрос \ref{query3} для датасета MOOC User Actions.}
\inputminted[frame=single,fontsize=\footnotesize,linenos,breaklines,xleftmargin=1.5em, breaksymbol=""]{text}
{lst/mooc/query3}
...
\begin{listing}[!htb]
\caption{Запрос \ref{query4} для датасета MOOC User Actions.}
\inputminted[frame=single,fontsize=\footnotesize,linenos,breaklines,xleftmargin=1.5em, breaksymbol=""]{text}
{lst/mooc/query4}
...
\begin{listing}[!htb]
\caption{Запрос \ref{query5} для датасета MOOC User Actions.}
\inputminted[frame=single,fontsize=\footnotesize,linenos,breaklines,xleftmargin=1.5em, breaksymbol=""]{text}
{lst/mooc/query5}
...
Исходные данные представляются обыкновенным неориентированным графом. DQL-схема датасета приводится в
...
\begin{listing}[!htb]
\caption{DQL-схема датасета California Road Network.}
\inputminted[frame=single,fontsize=\footnotesize,linenos,breaklines,xleftmargin=1.5em, breaksymbol=""]{text}
{lst/roadnet/schema}
...
\begin{listing}[!htb]
\caption{Запрос \ref{query1} для датасета California Road Network.}
\inputminted[frame=single,fontsize=\footnotesize,linenos,breaklines,xleftmargin=1.5em, breaksymbol=""]{text}
{lst/roadnet/query1}
...
\begin{listing}[!htb]
\caption{Запрос \ref{query2} для датасета California Road Network.}
\inputminted[frame=single,fontsize=\footnotesize,linenos,breaklines,xleftmargin=1.5em, breaksymbol=""]{text}
{lst/roadnet/query2}
...
\begin{listing}[!htb]
\caption{Запрос \ref{query3} для датасета California Road Network.}
\inputminted[frame=single,fontsize=\footnotesize,linenos,breaklines,xleftmargin=1.5em, breaksymbol=""]{text}
{lst/roadnet/query3}
...
\begin{listing}[!htb]
\caption{Запрос \ref{query4} для датасета California Road Network.}
\inputminted[frame=single,fontsize=\footnotesize,linenos,breaklines,xleftmargin=1.5em, breaksymbol=""]{text}
{lst/roadnet/query4}
...
\begin{listing}[!htb]
\caption{Запрос \ref{query5} для датасета California Road Network.}
\inputminted[frame=single,fontsize=\footnotesize,linenos,breaklines,xleftmargin=1.5em, breaksymbol=""]{text}
{lst/roadnet/query5}
\label{lst:roadnet:query5}
\end{listing}

\subsection{Датасет Stablecoin ERC20 Transactions} \label{datasetERC20}
...
\begin{listing}[!htb]
\caption{DQL-схема датасета Stablecoin ERC20 Transactions.}
\inputminted[frame=single,fontsize=\footnotesize,linenos,breaklines,xleftmargin=1.5em, breaksymbol=""]{text}
{lst/erc20/schema}
...
\begin{listing}[!htb]
\caption{Вспомогательный запрос для определения адреса отправителя с наибольшим числом передач.}
\inputminted[frame=single,fontsize=\footnotesize,linenos,breaklines,xleftmargin=1.5em, breaksymbol=""]{text}
...
\begin{listing}[!htb]
\caption{Запрос \ref{query1} для датасета Stablecoins ERC20 Transcations.}
\inputminted[frame=single,fontsize=\footnotesize,linenos,breaklines,xleftmargin=1.5em, breaksymbol=""]{text}
{lst/erc20/query1}
...
стейблкоинов.
\begin{listing}[!htb]
\caption{Запрос \ref{query2} для датасета Stablecoins ERC20 Transcations.}
\inputminted[frame=single,fontsize=\footnotesize,linenos,breaklines,xleftmargin=1.5em, breaksymbol=""]{text}
{lst/erc20/query2}
...
\begin{listing}[!htb]
\caption{Вспомогательный запрос для определения адреса контракта с наибольшим числом передач.}
\inputminted[frame=single,fontsize=\footnotesize,linenos,breaklines,xleftmargin=1.5em, breaksymbol=""]{text}
...
\begin{listing}[!htb]
\caption{Запрос \ref{query3} для датасета Stablecoins ERC20 Transcations.}
\inputminted[frame=single,fontsize=\footnotesize,linenos,breaklines,xleftmargin=1.5em, breaksymbol=""]{text}
{lst/erc20/query3}
...
\begin{listing}[!htb]
\caption{Запрос \ref{query4} для датасета Stablecoins ERC20 Transcations.}
\inputminted[frame=single,fontsize=\footnotesize,linenos,breaklines,xleftmargin=1.5em, breaksymbol=""]{text}
{lst/erc20/query4}
...
\begin{listing}[!htb]
\caption{Запрос \ref{query5} для датасета Stablecoins ERC20 Transcations.}
\inputminted[frame=single,fontsize=\footnotesize,linenos,breaklines,xleftmargin=1.5em, breaksymbol=""]{text}
{lst/erc20/query5}
